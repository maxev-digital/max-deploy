...
import { BookOpen, Sparkles, Loader2, Link, FileText, ChevronDown, ChevronRight, Wrench } from 'lucide-react';
...
export default function PrepStudioPage() {
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: 900 }}>
      ...
      {/* Input card */}
      <div style={{ background: cb, border: `1px solid ${bd}`, borderRadius: 14, padding: '28px', marginBottom: 32 }}>
      ...
      {result && (
        <div style={{ animation: 'fadeUp 0.3s ease' }}>
          ...
          {/* Fit score banner */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginBottom: 24, padding: '20px 24px', background: cb, border: `1px solid ${bd}`, borderRadius: 14 }}>
            <div style={{ textAlign: 'center', flexShrink: 0 }}>
              <div style={{ fontSize: '3rem', fontWeight: 900, lineHeight: 1, color: FIT_COLOR(result.fitScore) }}>{result.fitScore}</div>
              <div style={{ fontSize: '0.6rem', color: dim, marginTop: 2 }}>fit score</div>
            </div>
            <div style={{ borderLeft: `1px solid ${bd}`, paddingLeft: 20, flex: 1 }}>
              ...
              <div style={{ fontSize: '0.8rem', color: dim, marginBottom: 10 }}>{role}</div>
            ...
            {result.reasoning && (
              <div style={{ flex: 2, fontSize: '0.82rem', color: 'rgba(255,255,255,0.65)', lineHeight: 1.6, padding: '12px 16px', background: 'rgba(255,255,255,0.03)', borderRadius: 8, borderLeft: `3px solid ${FIT_COLOR(result.fitScore)}` }}>
                {result.reasoning}
              </div>
            )}
          </div>
          ...
          {/* Tabs */}
          <div style={{ display: 'flex', gap: 4, marginBottom: 20, borderBottom: `1px solid ${bd}`, paddingBottom: 12 }}>
            ...
            <button style={tabStyle('round2')} onClick={() => setActiveTab('round2')}>
              Round 2 — Technical
              {(result.deepDiveQuestions?.length ?? 0) > 0 && (
          ...
          {activeTab === 'overview' && (
            <div style={{ animation: 'fadeUp 0.3s ease' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
                <div style={{ background: cb, border: `1px solid ${bd}`, borderRadius: 12, padding: '16px 18px' }}>
                  ...
                  {(result.strengths ?? []).map((s, i) => (
                    <div key={i} style={{ display: 'flex', gap: 7, marginBottom: 7 }}>
                      <span style={{ color: '#14B8AD', flexShrink: 0, fontWeight: 700 }}>+</span>
                      <span style={{ fontSize: '0.78rem', color: 'rgba(255,255,255,0.7)', lineHeight: 1.45 }}>{s}</span>
                    </div>
                  ))}
                </div>
                <div style={{ background: cb, border: `1px solid ${bd}`, borderRadius: 12, padding: '16px 18px' }}>
                  ...
                  {(result.gaps ?? []).length === 0
                    ? <div style={{ fontSize: '0.78rem', color: '#14B8AD' }}>No material gaps.</div>
                    : (result.gaps ?? []).map((g, i) => (
                      <div key={i} style={{ display: 'flex', gap: 7, marginBottom: 7 }}>
                        <span style={{ color: '#D08E14', flexShrink: 0, fontWeight: 700 }}>—</span>
                        <span style={{ fontSize: '0.78rem', color: 'rgba(255,255,255,0.7)', lineHeight: 1.45 }}>{g}</span>
                      </div>
                    ))
                  }
                </div>
              ...
              {/* Talking points + cover letter angle */}
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
              ...
              {result.gapFraming && Object.keys(result.gapFraming).length > 0 && (
                <div>
                  <div style={{ fontSize: '0.62rem', fontWeight: 700, color: '#C8C8C8', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 12 }}>Gap Scripts — How to Answer</div>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(360px, 1fr))', gap: 12 }}>
                    {Object.entries(result.gapFraming).map(([gap, script], i) => (
                      <div key={i} style={{ padding: '14px 16px', background: 'rgba(208,142,20,0.06)', border: '1px solid rgba(208,142,20,0.15)', borderRadius: 10 }}>
          ...
          {activeTab === 'round2' && (
            <div style={{ animation: 'fadeUp 0.3s ease' }}>
              ...
              {(result.deepDiveQuestions ?? []).length === 0 ? (
                <div style={{ fontSize: '0.8rem', color: dim, paddingTop: 20 }}>No deep dive questions generated.</div>
